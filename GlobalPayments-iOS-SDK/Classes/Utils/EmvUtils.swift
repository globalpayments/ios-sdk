...
class EmvUtils {
    ...
    private var knownTags: [String: String] = [:]
    private var blackList: [String: String] = [:]
    private var dataTypes: [String: String] = [:]

    static let shared = EmvUtils()
    ...
    func parseTagData(_ tagData: String?, verbose: Bool = false) -> EmvData? {
        ...
        for var i in 1...tagData.count {
            ...
            if let blackTagName = blackList[tagName] {
                ...
                let approvedTag = TlvData(tag: tagName, length: lengthStr, value: value, description: knownTags[tagName] ?? "")
                if tagName == "5F28" && value != "840" {
                    rvalue.setStandInStatus(false, reason: "Card is not domestically issued")
                } else if tagName == "95" {
        ...
        if verbose {
            print("Accepted Tags:")
            for tagName in rvalue.tlvData.keys {
                if let tag = rvalue.getTag(tagName) {
//                    let appendBinary = dataTypes[tagName]
                    print("TAG: \(tagName) - \(tag.description)")
                    let appendString = " [\(tag.getBinaryValue() ?? "")]"
                    print("\(tag.length): \(tag.value)\(appendString)\r\n")
                }
            }
            print("Removed Tags:")
            for tagName in rvalue.removedTags.keys {
                if let tag = rvalue.removedTags[tagName] {
                    print("TAG: \(tagName) - \(tag.description)")
                    print("\(tag.length): \(tag.value)\r\n")
                }
