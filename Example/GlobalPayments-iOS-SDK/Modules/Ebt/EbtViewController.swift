...
final class EbtViewController: BaseViewController<EbtViewModel> {
...
extension EbtViewController: EbtViewDelegate {
    
    func onChargePressed() {
        viewModel?.doEbtTransaction(path: .charge)
    }
    
    func onRefundPressed() {
        viewModel?.doEbtTransaction(path: .refund)
    }
